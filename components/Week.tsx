...
export default function Week() {
  ...
  const openUpdateSheet = () => {
    ...
    Animated.timing(fadeAnimUpdate, {
      toValue: 1,
      duration: 250,
      useNativeDriver: true,
    }).start();
  ...
  const closeUpdateSheet = () => {
    ...
    Animated.timing(fadeAnimUpdate, {
      ...
      duration: 250,
      useNativeDriver: true,
    }).start(() => setIsUpdateSheetOpen(false));
  };
  ...
  const getSelectedItem = () =>
    headerItems.find((item) => item.id === selectedHeaderItemId);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        {headerItems.map((item) => (
          <TouchableOpacity
            ...
            style={[
              styles.headerItem,
              item.id === selectedHeaderItemId && styles.selectedHeaderItem,
            ]}
                ...
                >
                  <View>
                    ...
                    {task.description.length > 0 && (
                      <Text style={styles.selectedItemTasksText}>
                        {task.description}
                      </Text>
                    )}
      ...
      {isBottomSheetOpen && (
        <Animated.View
          style={[styles.bottomSheetContainer, { opacity: fadeAnim }]}
        >
      ...
      {isUpdateSheetOpen && (
        <Animated.View
          style={[styles.bottomSheetContainer, { opacity: fadeAnimUpdate }]}
        >
          <TouchableWithoutFeedback onPress={closeUpdateSheet}>
            <View style={styles.backdrop} />
          </TouchableWithoutFeedback>
          <BottomSheet
            ref={updateSheetRef}
            snapPoints={updateSnapPoints}
            enablePanDownToClose
            keyboardBehavior="interactive"
            keyboardBlurBehavior="restore"
            android_keyboardInputMode="adjustPan"
          ...
          >
            <BottomSheetView style={{ flex: 1, padding: 16, paddingBottom: 0 }}>
              <View style={styles.containerBottom}>
                <View style={styles.content}>
                  <Text style={styles.headerText}>
                    Görev: {selectedTask?.title}
                  </Text>
                  <BottomSheetTextInput
                    style={styles.input}
                    placeholder="Başlık"
                    value={taskTitleForUpdate}
                    onChangeText={setTaskTitleForUpdate}
                  />
                  <BottomSheetTextInput
                    style={styles.input}
                    placeholder="Açıklama"
                    value={taskDescriptionForUpdate}
                    onChangeText={setTaskDescriptionForUpdate}
                    multiline
                  />
                  <TouchableOpacity
                    style={styles.timeButton}
                    onPress={() => setShowTimePickerUpdate(true)}
                  >
                    <Text style={styles.timeText}>
                      Bitiş saati:{" "}
                      {dueTime.toLocaleTimeString([], {
                  ...
                  {showTimePickerUpdate && (
                    <DateTimePicker
                      value={dueTime}
                      mode="time"
                      is24Hour
                      display="spinner"
                      onChange={onChangeTimeUpdate}
                    />
                  )}
                </View>
                <View style={styles.actions}>
